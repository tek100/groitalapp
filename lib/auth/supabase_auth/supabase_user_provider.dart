...
class DocitalAppSupabaseUser extends BaseAuthUser {
  DocitalAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> docitalAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DocitalAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
